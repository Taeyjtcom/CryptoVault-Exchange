...
const BalanceCard = ({ label, amount, currency, trend }: BalanceCardProps) => (
  <div className="bg-slate-800/50 border border-slate-700/50 p-6 rounded-2xl flex flex-col gap-2 relative overflow-hidden group">
    ...
    <div className="flex items-baseline gap-2 z-10">
      <span className="text-2xl font-bold text-white">
        ...
        {currency !== 'USD' && (
          <span className="text-sm ml-1 text-slate-500">{currency}</span>
        )}
      </span>
    </div>
    <div
...
const LegacyAdminPanel = ({ config, setConfig }: AdminPanelProps) => {
...
type DepositModalProps = {
  ...
  const LegacyDepositModal = ({ isOpen, onClose, user, config }: DepositModalProps) => {
        ...
        {/* Header */}
          <header className="h-16 border-b border-slate-800/50 flex items-center justify-between px-8 bg-slate-950/50 backdrop-blur-md">
            <div className="flex items-center gap-4">
              ...
              {!isAdminMode && (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-500">Active client:</span>
                  <select
          ...
          <div className="bg-amber-500/10 border-b border-amber-500/40 text-amber-300 text-xs px-8 py-2 flex justify-between items-center">
            <span className="font-semibold">POC only – do not use for real funds.</span>
            <span className="hidden sm:inline text-amber-200/80">
              Addresses are derived from XPUBs in your browser for demonstration purposes only.
            </span>
          </div>
